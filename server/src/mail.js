...
const sendOrderConfirmation = (email, first_name, last_name, phone, message, cb) => {
  const mailOptions = {
    from: "restarter.dev@gmail.com",
    to: email,
    bcc: "restarter.dev@gmail.com",
    subject: `Los Girasoles Pedido Web - ${first_name} ${last_name}`,
    html: `<h1>Los Girasoles</h1><h2>Pedido Web ${today}</h2><p><ul><li>Nombre: ${first_name}</li><li>Apellido: ${last_name}</li><li>Telefono: ${phone}</li><li>Email: ${email}</li></ul></p><h2>Mensaje</h2><p>${message}</p>`,
  };
  transporter.sendMail(mailOptions, function (err, data) {
    if (err) {
      cb(err, null);
